...
function ENT:Use(ply, caller)
	...
	if( ply.BilliardTableID != nil && ply.BilliardTableID != self.ID ) then return
	elseif(ply.BilliardTableID == self.ID) then -- Maybe he wants to exit?
	...
	if(self.PlayerData[1] != nil) then
		local b_error = nil
		if(self.PlayerData[2] != nil) then b_error = "The billiard table is full!"
		elseif(self.Training) then b_error = "The billiard table is in Training mode!"
		elseif(self:IsPlayerInList(ply:UniqueID())) then b_error = "A request has already been sent!" end
		if(b_error != nil) then
			GAMEMODE:PlayerMessage( ply, "Error!", b_error )
			return
		elseif(!hook.Call("CanPlayerEnterBilliard", nil, self, ply)) then return end
...
function ENT:Think()
	...
	if(!self:GetTableEnt()) then return true end
...
function ENT:StartBilliardGame()
	...
	for i = 0, tmpDat.c do
		...
		elseif(self.GameType == BILLIARD_GAMETYPE_SNOOKER) then
			...
			elseif(i >= 16) then
				...
				ball.DontRemove = true
				ball.Value = i - 14
				ball:SetColor(self:GetColorByName(tmpDat.t[i - 15]))
			end
	...
	if(self.FPerson) then self:GetTurnPlayer():BilliardSpectate(true, cue, self:GetTableEnt()) end
...
function ENT:GetColorByName(name)
	...
	if(name == "yellow") then return 255,255,0,255
	elseif(name == "green") then return 0,160,25,255
	elseif(name == "brown") then return 90,60,20,255
	elseif(name == "blue") then return 0,80,175,255
	elseif(name == "pink") then return 255,100,255,255
	elseif(name == "black") then return 0,0,0,255 end
...
function ENT:SetConfig(c_gm, c_tm, c_tr, c_sc, c_ab, c_mp, c_fp)
	if(self:GetTableEnt():IsValid()) then return end -- Changing config while game is running is not allowed!
