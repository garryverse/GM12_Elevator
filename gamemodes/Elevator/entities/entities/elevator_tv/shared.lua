...
if SERVER then
	...
	function ENT:TVIsPlaying()
		return tobool( GAMEMODE.VideoID )
	end
	
	function ENT:TVTime()
		return math.Round( CurTime() - ( GAMEMODE.VideoStartTime or 0 ) )
	end
	...
	function ENT:SetTV( ply, vid, dur )
		...
		GAMEMODE.VideoID = vid
		GAMEMODE.VideoStartTime = CurTime()
	...
	function ENT:UpdatePlayer( ply )
		...
		umsg.Start( "elevator_updatetv", ply )
			umsg.Entity( self )
			umsg.String( GAMEMODE.VideoID or "" )
	...
	concommand.Add( "elev_settv", function( ply, cmd, args )
		...
		if !ValidEntity( tv ) then return end
	...
	function ENT:DisplayControls()
		...
		self.HTMLFrame.CloseSafe = function( self )
			DFrame.Close( self )
...
if CLIENT then
	...
	function PANEL:OpeningURL( url, target, postdata, postdataset )
		...
		local ok, _, match = string.find( url , "id=([%w_-]+)")
		
		if DEBUG then 
			print("Open URL: ", url, ok, match ) 
		end
		
		if ok && IsValid( self.Entity ) then
			if match != GAMEMODE.VideoID then

				// Update clients
				RunConsoleCommand( "elev_settv", match, self.Entity:EntIndex() )

				// Store vid
				GAMEMODE.VideoID = match

				// Update starter's TV
				self.Entity:UpdateTV( match, 0 )
			end

			// Close starter's TV
			if ( ValidPanel( self.Entity.HTMLFrame ) ) then
				self.Entity.HTMLFrame:CloseSafe()
			end
		end

	end

	function PANEL:FinishedURL( url ) end
	function PANEL:StatusChanged( status ) end

	vgui.Register( "EntityHTML", PANEL, "HTML" )
