	...
	function ENT:Think()
		...
		if ( IsValid( grabbedNPC ) ) then
			...
			if ( GAMEMODE:IsGirl( grabbedNPC:GetModel() ) ) then
				WorldSound( "elevator/effects/hand_grab.mp3", pos, 500, 160 )
			else
				WorldSound( "elevator/effects/hand_grab.mp3", pos, 500, 100 )
			end
		else
			WorldSound( "elevator/effects/hand_miss.mp3", pos, 500, 100 )
		end
