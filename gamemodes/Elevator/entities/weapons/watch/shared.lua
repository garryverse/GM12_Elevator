...
function SWEP:Slap()
	...
	self.Owner:SetAnimation(ACT_MELEE_ATTACK1) -- need to use crowbar attack or something
	...
	if tr.Hit then
		...
		if IsValid(ent) && ( ent:IsPlayer() || ent:IsNPC() || ent:GetClass() == "prop_physics" ) then
			...
			local dmginfo = DamageInfo()
				...
				local vec = (tr.HitPos - pos):Normalize()
...
function SWEP:PrimaryAttack()
	...
	if GAMEMODE:WasHoldingObject(self.Owner) then return end -- don't view watch while holding object
...
--[[-----------------------------------------
	Fake third person animations
-------------------------------------------]]
if SERVER then

	hook.Add("PlayerInitialSpawn", "SendFakeWatchAnim", function(ply)
		timer.Simple( 1, function()    
			umsg.Start( "SetFakeWatchAnim" )
			umsg.End()
		end )
	end)
	...
	usermessage.Hook("SetFakeWatchAnim", function(um)
		for _, ply in ipairs(player.GetAll()) do
			ply.BuildBonePositions = function()
				...
				if !watch.IsViewingWatch then return end -- for some silly reason, I have to check this
				if !watch:IsViewingWatch() then return end
				...
				if ( boneIndex ) then
					...
					if ( boneMatrix ) then
						boneMatrix:SetAngle(Angle(0,-20,0))
						boneMatrix:Translate(Vector(5,1,0))
						ply:SetBoneMatrix( boneIndex, boneMatrix )
					end
				end
				
				-- Rotate Upperarm
				local boneIndex = ply:LookupBone( "ValveBiped.Bip01_R_UpperArm" )
				local boneMatrix = ply:GetBoneMatrix( boneIndex )
				boneMatrix:SetAngle(Angle(45,-40,90))
				boneMatrix:Translate(Vector(1,6,-3))
				ply:SetBoneMatrix( boneIndex, boneMatrix )
				
				-- Rotate Forearm
				local boneIndex = ply:LookupBone( "ValveBiped.Bip01_R_Forearm" )
				local boneMatrix = ply:GetBoneMatrix( boneIndex )
				boneMatrix:SetAngle(Angle(-60,-100,-60))
				boneMatrix:Translate(Vector(-3,7,13))
				ply:SetBoneMatrix( boneIndex, boneMatrix )
			end
		end
	end)
