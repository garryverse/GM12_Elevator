...
local OPACITY_CVAR = CreateClientConVar( "elev_opacitydistance", 30, false, false )

surface.CreateFont( "default", 35, 700, true, false, "HudCText" )
surface.CreateFont( "default", 18, 700, true, false, "HudCSubText" )
...
local function CreateMusic( id )
	...
	LocalPlayer().Music = CreateSound( LocalPlayer(), GAMEMODE.Music[ id ] )
...
function GM:PlayerBindPress( ply, bind, pressed )
	...
	if IsValid( watch ) then
		if ( watch:IsViewingWatch() && bind == "+zoom" ) then
			return true
		end
	end
...
function GM:PrePlayerDraw( ply )
	...
	// Only apply on players in the elevator
	if ply:Team() != TEAM_ELEVATOR then return end

	local dist = ( ply:GetPos() - LocalPlayer():GetPos() ):Length()
	local opacity = math.Clamp( ( dist / OPACITY_CVAR:GetFloat() ) - 0.2, 0, 1)
	if opacity < 1 then
		ply.RenderGroup = RENDERGROUP_TRANSLUCENT
	end
	
	render.SetBlend( opacity )
...
function GM:PostPlayerDraw( ply )
	...
	ply.RenderGroup = RENDERGROUP_BOTH
	render.SetBlend(1) -- opaque
