...
function GM:SetNewSong()
	...
	self.CurrentSongTime = CurTime() + self.MusicLength
...
function GM:OnEntityCreated( ent )
	...
	// Give them a name
	timer.Simple( .1, self.SetNPCName, self, ent ) // we need to delay this a bit
...
function GM:SetFloorEffects( FloorName, bEnable )
	...
	if ( FloorName == "elevator_6" ) then
		...
		if bEnable then
			...
			timer.Create( "CitizenCheering", math.random( 1, 3 ), 0, function()
				...
				timer.Adjust( "CitizenCheering", math.random( 1, 3 ), 0 ) // Loop at random interval
			end)
